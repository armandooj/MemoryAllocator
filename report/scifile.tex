...
\begin{tikzpicture}[node distance = 4cm, auto]
    % Place nodes
    \node [block] (init) {index.html: call doGet};
    \node [decision, below of=init] (doGet) {doGet: parameter are setted?};
    \node [block, left of=doGet] (form) {Display the form to the user};
    \node [block, below of=form] (submit) {submit using doPost};
    \node [block, right of=submit] (doPost) {doPost: calls doGet};
    \node [block, right of=doGet] (show) {Display the message};
    % Draw edges
    \path [line] (init) -- (doGet);
    \path [line] (doGet) -- node {no} (form);
    \path [line] (form) -- (submit);
    \path [line] (submit) -- (doPost);
    \path [line] (doPost) -- (doGet);
    \path [line] (doGet) -- node {yes} (show);
